...
class EntitySQL:
    ...
    def field_config(self, etid=1431):
        query = f"boxer_entities.dbo.[spi_Entities_GetEntityTypeSchemaByEntityTypeID] {etid},'deepp'"
        return self.db.execQuery(query)
    def spi_test(self):
        ''' testing endpoint for stored procedures'''
        query = "execute boxer_entities.dbo.spi_Entities_GetEntityTypeList 'karnas' , null"
        return self.db.execQuery(query)

    def type_list_by_id(self, id):
        ''' Id is an applicaiton id'''
        query = f''' SELECT E.ENTITY_ID as [ID],FIELD_VALUE as Entity_Types, 
        EXTERNAL_DATASOURCE_OBJECT_ID as [EID]--, etid.TEXT as [ETID]
