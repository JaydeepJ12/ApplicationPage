...
export default function EntityInfoBlock(props) {
  ...
  if (!props.dataInfoLoaded && !props.noDataFound) {
    return <div>No Record Found</div>;
  } else if (!props.dataInfoLoaded) {
    return <div>loading............</div>;
  } else {
    return (
      <div
        className="page-entity-info"
        style={{ height: "100%", width: "100%" }}
      >
        <Grid container>
          <form
            className={classes.form_root + " MuiGrid-root MuiGrid-container"}
          >
            {(!props.dataInfoLoaded
              ? Array.from(
                  new Array(
                    props.entityInfoData.length
                      ? props.entityInfoData.length
                      : 10
                  )
                )
